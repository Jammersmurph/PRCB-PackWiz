...
public class Bootstrap {
	...
	private static final String DEFAULT_UPDATE_URL = "https://api.github.com/repos/Jammersmurph/packwiz-installer/releases/latest";
	private static final String RELEASE_ASSET_NAME = "createvc-updater.jar";
	public static final String JAR_NAME = "prcb-packwiz-installer.jar";
	...
	public static void init(String[] args) {
		...
		if (skipUpdate) {
			try {
				...
				showError(e, "Project Crowbar Packwiz installer cannot be found, or there was an error loading it:");
				System.exit(1);
			} catch (Exception e) {
				showError(e, "There was an error loading the Project Crowbar Packwiz installer:");
				System.exit(1);
			}
			return;
		}
		...
		try {
			...
			showError(e, "There was an error downloading the Project Crowbar Packwiz installer:");
		}
		...
		if (useGUI) {
			...
			try {
				EventQueue.invokeAndWait(() -> JOptionPane.showMessageDialog(null,
					...
					"Project Crowbar Updater", JOptionPane.ERROR_MESSAGE));
	...
	private static void parseOptions(String[] args) throws ParseException {
		...
		options.addOption(null, "bootstrap-no-update", false, "Don't update the Project Crowbar Packwiz installer");
		options.addOption(null, "bootstrap-main-jar", true, "Location of the Project Crowbar Packwiz installer JAR file");
		...
		if (cmd.hasOption("help")) {
			...
			// Add options from the Packwiz installer JAR, if it is present.
			boolean jarLoaded = LoadJAR.addOptions(options, jarPath);
			formatter.printHelp("java -jar prcb-updater-bootstrap.jar", options);
			if (!jarLoaded) {
				System.out.println("Packwiz installer options will be visible once it has been downloaded.");
			}
			System.exit(0);
		}
	...
	private static Release requestRelease() throws IOException, GithubException {
		...
		InputStream in;
		if (useGUI) {
			in = new ConnMonitorInputStream(conn, "Checking for Project Crowbar updater updates...", null);
		} else {
		...
		for (JsonValue assetValue : assets.asArray()) {
			...
			if (!name.equalsIgnoreCase(RELEASE_ASSET_NAME)) {
				continue;
			}
	...
	private static void downloadUpdate(String downloadURL, String assetURL, String path) throws IOException {
		...
		InputStream in;
		if (useGUI) {
			in = new ConnMonitorInputStream(conn, "Updating the Project Crowbar Packwiz installer...", null);
		} else {
			in = conn.getInputStream();
		}
